...
(function () {
  ...
  function addUspapiFrame() {
    if (!window.frames["__uspapiLocator"]) {
      if (document.body) {
        ...
        iframe.setAttribute(
          "style",
          "display:none;height:0;opacity:0;visibility:hidden;border:0;width:0;height:0;",
        );
  ...
  function optOutMsgHandler(event) {
    ...
    if (msgIsString) {
      json =
        event.data.indexOf("__uspapiCall") !== -1 ? JSON.parse(event.data) : {};
    } else {
      json = event.data;
    }
  ...
  window.usPrivacyCookie = (function () {
    ...
    function createCookie(name, value, days, expiryDate) {
      ...
      var cookieString =
        nameString + expireString + samesiteString + secureString;
  ...
  window.gamoo = (function () {
    ...
    function createCookie(name, value, days) {
      ...
      document.cookie =
        nameString + expireString + pathString + samesiteString + secureString;
    }
    ...
    function enableGam() {
      if (typeof googletag !== "undefined" && googletag.apiReady) {
        ...
        googletag
          .pubads()
          .setPrivacySettings({ restrictDataProcessing: false });
    ...
    function init() {
      ...
      function OptOutPrivacyObject() {
        ...
        function returnValue() {
          return (
            privacyStringValues.version +
            privacyStringValues.notice +
            privacyStringValues.optout +
            privacyStringValues.lspa
          );
        }
      ...
      if (isLocationApplicable()) {
        ...
        function groupsUpdated() {
          ...
          if (consent && consent.USPrivacy && consent.consentedDate) {
            usPrivacyCookie.create(
              consent.USPrivacy,
              new Date(consent.consentedDate),
            );
          } else {
            ...
            var activeGroupsArr = !OnetrustActiveGroups.slice(1, -1)
              ? []
              : OnetrustActiveGroups.slice(1, -1).split(",");
      ...
      function getCCPAOptOutIds() {
        ...
        for (var i = 0, l = scripts.length; i < l; i++) {
          var scriptHasCCPAOptOutIds =
            !!scripts[i].attributes.getNamedItem("ccpa-opt-out-ids");
          if (scriptHasCCPAOptOutIds) {
            // get the value of the "ccpa-out-out-ids" attribute
            var ccpaOptOutValue =
              scripts[i].attributes.getNamedItem("ccpa-opt-out-ids").value;
            geoValue =
              scripts[i].attributes.getNamedItem("ccpa-opt-out-geo").value;
            lspaValue =
              scripts[i].attributes.getNamedItem("ccpa-opt-out-lspa").value;
